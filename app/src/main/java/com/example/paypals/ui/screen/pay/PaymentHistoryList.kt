...
fun PaymentHistoryList(
     paymentsWithUserNames: List<PaymentWithUser>,
     onEditDate: (Payment, Long) -> Unit
) {
     ...
     if (selectedPayment != null) {
          DatePickerDialog(
               ...
               confirmButton = {
                    TextButton(onClick = {
                         datePickerState.selectedDateMillis?.let { newTimestamp ->
                              // Preservar la hora original
                              val calendar = java.util.Calendar.getInstance()
                              calendar.timeInMillis = newTimestamp

                              val oldCalendar = java.util.Calendar.getInstance()
                              oldCalendar.timeInMillis = selectedPayment!!.timestamp

                              calendar.set(java.util.Calendar.HOUR_OF_DAY, oldCalendar.get(java.util.Calendar.HOUR_OF_DAY))
                              calendar.set(java.util.Calendar.MINUTE, oldCalendar.get(java.util.Calendar.MINUTE))

                              onEditDate(selectedPayment!!, calendar.timeInMillis)
                         }
     ...
     LazyColumn(
          ...
          items(paymentsWithUserNames) { item ->
               PaymentHistoryItem(
                    paymentWithUser = item,
                    onClick = {
                         selectedPayment = item.payment
                         datePickerState.selectedDateMillis = item.payment.timestamp
                    }
               )
               HorizontalDivider()
          }
